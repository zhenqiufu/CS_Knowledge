/*基于双船单独DP系统基础上的高级控制器,利用单船的控制器;
  设置一个按钮,当两船就位以后,将权限接管到高级控制器,
  无论当前工作模式如何,强制设置为0偏DP;
  设置偏置位置,3Dof,需要以某一个船为基础,另一个船进行相对定位;
  开始协同控制
  协同控制算法:非一致性PID\基于一致性原理的PID
  ...
  一致性PID:订阅两船当前状态,与目标点,计算出误差,进行PID,
            同时计算相对位置加上一致性项,pub两船推力
...
int main(int argv, char **argc) {
  ...
  ros::Subscriber sub_vessel_data =
      n.subscribe("kalman", 1, simulator_callback);
  ...
  while (ros::ok()) {
    ...
    if (!comparevelocityerror(V_error)) {
      for (int i = 0; i < 3; ++i) {
        _tau(i) += V_PID_P[i] *
                   V_error(i);  // proportional term
                                //  + V_PID_I[i] * position_error_integral(i);
                                //  // integral term
      }
    }
    //赋值
